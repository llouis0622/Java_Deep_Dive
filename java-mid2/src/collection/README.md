...
- 정렬 알고리즘 : 듀얼 피벗 퀵소트(데이터 적을 때), 팀소트(데이터 많을 때)
- Comparator : 두 값을 비교할 때 비교 기준을 직접 제공 가능
- Comparable : 객체에 비교 기능 추가

### 3. 컬렉션 프레임워크 전체 정리
- 일관성 : 모든 컬렉션들이 기본적인 동작을 공유함
- 재사용성 : 다양한 컬렉션 타입들에 공통으로 적용
- 확장성 : 기존에 정의된 알고리즘과 도구 사용 가능
- 다형성 : 다양한 컬렉션 타입들을 같은 타입으로 다룰 수 있음